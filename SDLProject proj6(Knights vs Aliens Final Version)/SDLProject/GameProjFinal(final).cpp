...
void initialise()
{
    ...
    display_window = SDL_CreateWindow("Knight vs Aliens (The Final Version)",
...
void process_input()
{
    ...
    //if (current_scene != menu) current_scene->state.player->set_movement(glm::vec3(0.0f));
    if (current_scene != menu) current_scene->state.player->set_movement(glm::vec3(0.0f));
    //if (current_scene == win_screen || current_scene == loss_screen) current_scene->state.player->deactivate();
        ...
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_SPACE:
                        // Jump
                        //current_scene != titleScreen && current_scene->state.player->collided_bottom
                        if (current_scene != menu && current_scene->state.player->collided_bottom)
                        {
                        ...
                        break;
                    case SDLK_RETURN:
                        if (current_scene == menu) switch_to_scene(levelA);
    ...
    if (current_scene != menu && glm::length(current_scene->state.player->movement) > 1.0f)
    {
...
void update()
{
    ...
    while (delta_time >= FIXED_TIMESTEP) {
        ...
        if (current_scene != menu && is_colliding_bottom == false && current_scene->state.player->collided_bottom) effects->start(SHAKE, 1.0f);
        
        if (current_scene != menu) is_colliding_bottom = current_scene->state.player->collided_bottom;
    ...
    if (current_scene != menu && current_scene->state.player->get_position().x > LEVEL1_LEFT_EDGE) {
